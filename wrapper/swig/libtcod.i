...
#if SWIGLUA
%ignore TCODColor::TCODColor(uint8,uint8,uint8);
%ignore TCODColor::TCODColor(float,float,float);
#endif
...
#if SWIGLUA
%ignore TCODRandom::get(float,float);
%ignore TCODRandom::get(float,float,float);
%ignore TCODRandom::get(double,double);
%ignore TCODRandom::get(double,double,double);
%ignore TCODRandom::TCODRandom(TCOD_random_algo_t);
#endif
...
/* Lua callback wrappers. 
   Very incomplete - callbacks are objects GC'd by Lua, so the user must take care not to delete them prematurely.
 */
%include "lua_fnptr.i"
%{
...
// this will get translated into tcod.line.Listener, tcod.path.Callback
%rename(TCODLine_Listener) LuaLineListener;
%rename(TCODPath_Callback) LuaPathCallback;
